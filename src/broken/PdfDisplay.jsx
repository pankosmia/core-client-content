...
import DisplayForPdf from './DisplayForPdf';

function PdfDisplay({bookNames, repoSourcePath, open, closeFn}) {
    ...
    const pdfDisplayOneBook = async bookCode => {
    ...
    return <>
                ...
                <Button
                    onClick={() => {
                        if (!fileExport.current || fileExport.current.length === 0) {
                            ...
                            pdfDisplayOneBook(fileExport.current[0]).then();
                        }
                        closeFn();
                    }}
        ...
        {
            usfm && <DisplayForPdf usfmText={usfm}/>
        }
    </>;
}
