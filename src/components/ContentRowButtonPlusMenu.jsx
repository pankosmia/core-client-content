...
function ContentRowButtonPlusMenu({
    ...
    ([category, categoryValue]) => {
      ...
      return Object.entries(endpoints).flatMap(([, endpointValue]) => {
        ...
        return docs.map((doc) => ({
          ...
          url: '/clients/' + category + '#' + doc.url.replace('%%REPO_PATH%%', repoInfo.path),
        }));
      });
    }
  );
  ...
  const createItemExport = Object.entries(clientInterfaces).flatMap(([category, categoryValue]) => {
    const endpoints = categoryValue?.endpoints ?? {};

    return Object.entries(endpoints).flatMap(([endpointKey, endpointValue]) => {
      ...
      return exportsArray.flatMap((doc) => {
        ...
        return Object.entries(flavorItems).flatMap(([key, items]) =>
          items.map((item) => ({
            ...
            url: '/clients/' + category + '#' + item.url.replace('%%REPO_PATH%%', repoInfo.path),
          }))
        );
      });
    });
    ...
    <>
        ...
        {isNormal ? (
          <>
            {repoInfo.path.includes('_local_/_local_') && (
              <>
                {createItemNewBook.filter((item) => item.category === repoInfo.flavor).length >
                  0 && (
                  <>
                    {createItemNewBook
                ...
                {createItemImportBook.filter((item) => item.category === repoInfo.flavor).length >
                  0 && (
                  <>
            ...
            <MenuItem onClick={handleSubMenuClick}>
              <ListItemText>{doI18n('pages:content:export', i18nRef.current)}</ListItemText>
             ...
             */}
            <Divider />
            {repoInfo.path.includes('_local_/_local_') && createVersionManager.length > 0 && (
              <>
      ...
      >
        {createItemExport
          ...
          .map((item) => (
            <MenuItem key={item.label} onClick={() => (window.location.href = item.url)}>
              {item.label}
            </MenuItem>
          ))}
        <MenuItem
